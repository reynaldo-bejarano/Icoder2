...
const DateTableUser = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const res = await axios.get(`/api/auth/dates/view/user/${userID.id}`)
                ...
                >
                    ...
                    <tbody>
                        ...
                        {dates.length > 0 ? dates.map<any>((item: any) => {
                            return (
                                <tr className="text-slate-900 grid grid-cols-9 items-center text-left gap-2 border py-2 capitalize" key={item._id}>
